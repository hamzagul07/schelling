...
def _resolve_tags(
    ...
    def repl_group(m: re.Match[str]) -> str:
        ...
        # Provenance moved to an appendix table (Session 61, D61.x): the inline citation renders the
        # value in parentheses and no longer carries a `[^ev-...]` footnote marker — the appendix at
        # the end lists every cited figure's E-tag, value, source, and provenance stamp instead. The
        # `used` set still collects each cited tag so the table is complete.
        if marks:
            distinct = list(dict.fromkeys(marks))
            sentence = _current_sentence(m.string[: m.start()])
            # Value-only elision: drop the redundant parenthetical when EVERY cited value is already
            # stated in the sentence prose (D16.2), or when a lone value is a non-numeric
            # verdict/description — never a redundant "(351)" echo, a bare "(FAILED)"/"(188)", or a
            # "(23.84 vs 22.99)" already spelled out. The provenance still appears in the appendix.
...
def assemble(repo_root: Path) -> tuple[str, list[str], list[str]]:
    ...
    appendix = [
        "",
        "## Appendix: provenance of cited figures",
        "",
        "Every figure the paper cites is regenerated by `schelling paper-evidence` from the listed "
        "artifact; the E-tag keys each value to its source and provenance stamp.",
        "",
        "| E-tag | Value | Source | Provenance |",
        "|---|---|---|---|",
    ]
    for tag, item in used.items():
        appendix.append(f"| {tag} | {item['value']} | `{item['source']}` | `{item['prov']}` |")
